...
Note: Previously, we supported the 1.2.0 version of the Benchmark. If you are looking to upgrade from 1.2.0 to 1.3.0,
...
The CIS AWS Foundations Benchmark is organized into five sections:
 +
 +
...
The CIS v1.3 recommendations require that all expired SSL/TLS certificates stored in AWS IAM are automatically removed
(see 1.19). Removing expired SSL/TLS certificates eliminates the risk that an invalid certificate will be deployed
accidentally to a resource such as AWS Elastic Load Balancer (ELB), which can damage the credibility of the
application/website behind the ELB. As a best practice, it is recommended to delete expired certificates.

Unfortunately removing expired certificates via AWS Management Console is not currently supported so we must remove
...
To help you achieve this recommendation, we've create a Lambda function that runs on a regular schedule and
automatically removes all expired SSL/TLS certificates. Check out the link:https://github.com/gruntwork-io/cis-compliance-aws/tree/master/examples/cleanup-expired-certs/terraform[example]
provided in the Gruntwork Service Catalog to see how to deploy a ready-made module that can be
deployed in each of your AWS accounts.

[[iam_access_analyzer]]
==== IAM Access Analyzer
The latest CIS guide requires that the AWS IAM Access Analyzer service is enabled across all active regions in a given
...
your account. Alternatively, you could make use of the
link:https://github.com/gruntwork-io/module-security/blob/master/modules/iam-access-analyzer-multi-region[module]
provided in the Gruntwork Service Catalog.
...
=== Storage
The new (as of v1.3.0) Storage section has three recommendations pertaining to the S3 service as well as the EC2 service.
These have to do with encryption at rest and in transit.

To comply with the recommendation 2.1.1, make sure to enable server side encryption on your S3 buckets. In Terraform, this
is achieved by configuring the `server_side_encryption_configuration` argument of the `aws_s3_bucket` resource.

To comply with the recommendation 2.1.2, make sure that all access to your S3 buckets is over TLS. In Terraform, you will
...
Recommendation 5.1 forbids allowing access to the remote server administration ports, such as SSH to port 22 and Remote
Desktop to port 3389, from the public Internet (`0.0.0.0/0`) on the level of Network ACL rules. Recommendation 5.2 does
the same for Security Groups.  This seems entirely reasonable, no? Avoid creating any Security Groups and NACL with
these rules. Instead, if you require SSH or Remote Desktop to your cloud resources, provide a more restricted CIDR
range, such as the IP address of your offices.
...
TODO(Rob): Add instructions for 1.19.
...
link:https://github.com/gruntwork-io/aws-service-catalog/tree/master/examples/for-learning-and-testing/landingzone/account-baseline-root[account-baseline-root]
and link:https://github.com/gruntwork-io/aws-service-catalog/tree/master/examples/for-learning-and-testing/landingzone/account-baseline-security[account-baseline-security].

For the standalone module available as pat of the Gruntwork module library and Service Catalog, have a read here:
link:https://github.com/gruntwork-io/module-security/blob/master/modules/iam-access-analyzer-multi-region/README.adoc[README.adoc].
...
[[configure_storage]]
=== Configure storage
To make sure your S3 buckets are compliant with the benchmark, all you have to do is use the
link:https://github.com/gruntwork-io/module-security/tree/master/modules/private-s3-bucket[`private-s3-bucket` module].

TODO(Rob): Add instructions for 2.2.1.
...
link:https://github.com/gruntwork-io/cis-compliance-aws/tree/master/modules/vpc-app-network-acls[`vpc-app-network-acls`
module] and your NACL Rules for the VPC-Mgmt (which is being deprecated) using the 
link:https://github.com/gruntwork-io/cis-compliance-aws/tree/master/modules/vpc-mgmt-network-acls[`vpc-mgmt-network-acls`
module].
...
1.22,TODO,Use the security AWS account
2.1.1,<<configure_storage>>,TODO(Ana)
2.1.2,<<configure_storage>>,TODO(Ana)
2.2.1,<<configure_storage>>,TODO(Rob)
