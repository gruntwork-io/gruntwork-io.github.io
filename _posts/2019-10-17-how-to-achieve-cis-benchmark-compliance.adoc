...
[[manual_steps_storage]]
==== Manual steps
TODO: Add manual steps for macie
...
input = {
  ...
  # Use the following variables if you want to set up an example configuration.
  create_macie_bucket = true
  macie_bucket_name = "macie-bucket-example-cis-guide"
  macie_create_kms_key = true
  macie_kms_key_name = "macie_kms_key_example_cis_guide"
  macie_kms_key_users = ["arn:aws:iam::${local.accounts.security}:root","arn:aws:iam::${local.accounts.security}:user/alice", "arn:aws:iam::${local.accounts.security}:user/bob"]
  macie_buckets_to_analyze = {
    "eu-west-1" : ["PROVIDE_YOUR_OWN_BUCKETS_TO_ANALYZE_HERE"],
  }
}
----
