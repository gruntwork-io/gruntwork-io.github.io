...
variable "allow_administrative_remote_access_cidrs_public_subnets" {
  description = "A map of CIDR blocks that will be allowed to access to administrative ports (e.g., SSH, RDP) in the public subnet tier."
  type        = map(string)
...
CIS compliance, and as such they don't allow security groups to access to ports 22 or 3389 from the world. In addition,
...
3.9,<<vpc_flow_logs>>,Gruntwork's `vpc` service enables flow logs by default
...
5.1,<<configure_networking>>,Use the Gruntwork `vpc` service to ensure there is no public remote access
5.2,<<configure_networking>>,Use the Gruntwork `vpc` service for a secure network configuration
5.3,<<configure_networking>>,The `cloud-nuke` tool can remove all default security groups
5.4,<<configure_networking>>,Gruntwork's `vpc` service creates least-privilege routing by default
|===
