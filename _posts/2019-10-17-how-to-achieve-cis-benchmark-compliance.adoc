...
If your using Terragrunt, then you can invoke the standalone module directly by following the pattern outlined in
...
For the standalone module available as part of the Gruntwork module library and Service Catalog, you can include it in a `terragrunt.hcl` configuration as:

[source,hcl]
----
# ----------------------------------------------------------------------------------------------------------------------
# IAM ACCESS ANALYZER DEFAULTS
# This module uses a feature flag so it can be easily disabled or enabled using `enable_iam_access_analyzer` variable.
# ----------------------------------------------------------------------------------------------------------------------
module "iam_access_analyzer" {
  source = "../iam-access-analyzer-multi-region"
...
To see a usage example of the `private-s3-bucket` module, refer the
link:the https://github.com/gruntwork-io/terraform-aws-service-catalog/blob/master/modules/data-stores/s3-bucket/[`s3-bucket` service]
in the Gruntwork Service Catalog.
...
module "ebs_encryption" {
  source         = "../ebs-encryption-multi-region"
  aws_account_id = var.aws_account_id
  seed_region    = var.aws_region
