...
To further simplify and expedite compliance, the Gruntwork AWS CIS Service Catalog uses both standalone compliance
modules as well as "wrappers" around the core, unopinionated modules in the Infrastructure as Code Library. The wrappers
...
It is recommended that security questions are registered in the AWS account, so that they are used to authenticate individuals calling AWS customer service for support. To add your security
questions to the root account, navigate in the web console for AWS to `My Account` and then the `Personal Information` page.
There you should be able to click on `Configure Security Challenge Questions` and add your questions.

_This fulfils requirement #1.3 from the CIS 1.3.0 benchmark._
...
dramatically improves the security of your root user. If you use a virtual or hardware MFA device, it is recommended by the CIS benchmark that it is non-personal device, but instead company-owned or specifically provided for the purposes.

_Enabling virtual MFA fufils requirement #1.5 from the CIS 1.3.0 benchmark._
_Enabling hardware MFA fulfils requirement #1.6 from the CIS 1.3.0 benchmark._

It is up to your discretion which of the requirements you choose to comply with.
...
_This fulfils requirement #1.4 from the CIS 1.3.0 benchmark._
