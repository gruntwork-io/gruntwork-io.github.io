...
CIS recommendation 1.3 suggests that you register your AWS account with security questions. This is so that when someone contacts AWS customer service support from this account they will have to authenticate. To add your security questions to the root account, navigate in the web console for AWS to `My Account` and then the `Personal Information` page.
...
dramatically improves the security of your root user. It is up to your discretion to decide which option yis suitable for your use case. CIS 1.3 bechmark recommends that any virtual or hardware device you use to fulfil MFA is not personal, but instead dedicated or company-owned.
