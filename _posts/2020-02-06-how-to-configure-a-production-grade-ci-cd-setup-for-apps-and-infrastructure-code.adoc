...
CI/CD for infrastructure code is a large topic and a single guide cannot cover everything there is to the topic. There
...
Feature toggles::
  ...
  units. Major features can not be implemented in a day. How can you ship parts of your feature without breaking the
...
Live Infrastructure Config::
  ...
  infrastructure. To illustrate this point, consider a scenario where you are working on updating the cross account IAM
  ...
  The only real test you can do for live infrastructure config is to do a dry run of your infrastructure code. Most Infrastructure as Code tools support a dry run of the code to check what it would do against your environment. For example, with Terraform, you could run `terraform plan` to sanity check the planned actions Terraform will take. This is especially useful for sanity checking a fresh clone of the code. The trunk should be a true reflection of the live environment, so you should expect there to be no changes to make on a fresh clone of trunk.
...
hand, if you have no automated tests or working on an area that can't be tested automatically (e.g live infrastructure
code), then you'll want as many eyes as possible reviewing the code.
...
infrastructure vs Immutable infrastructure] from our blog post, _Why we use Terraform and not Chef, Puppet, Ansible,
SaltStack, or CloudFormation_ for a discussion on the benefits of immutable artifacts). What the release artifact looks
...
Application Code::
  ...
  your live infrastructure config. In most cases, the changes are simple enough that a single `sed` call should
...
Infrastructure Modules::
  ...
  application code, automating this process could be handled with a single `sed` call that updates the ref tag in the
  live infrastructure config, with the rollout being handled by the automated deployment pipeline for that code.

Live Infrastructure Config::
  Like with infrastructure modules, automatically deploying live infrastructure config without any oversight might be
  ...
  destroying your production database is not advisable. With that said, it is not practical to always manually roll out
  ...
  the change. In this way, the actual deployment steps are automated, but the trigger for deployment is manual, allowing
  you to pause and review the deployment to make sure nothing bad will happen.
...
Application Code::
  ...
  attack, but most likely will not affect databases and backups, making it recoverable after locking out the attacker.

Infrastructure Modules::
  Compromising the credentials used for infrastructure modules can cause an attacker to maliciously use your cloud
  environments for various purposes, potentially costing you lots of money. However, since the credentials do not grant
  access to your production (or even pre-prod) environment, it can't be used to take down your application or steal data.
  While this could cost your business financially, in most cases, you can recover the costs by reporting fraud. AWS for
  example is known to refund bills of accounts that were compromised.

Live Infrastructure Config::
  Compromising the credentials used for live infrastructure config has the highest consequences. Attackers with access
  to these credentials can do anything in any environment, including deleting all your backups. This is potentially a
...
https://circleci.com/[CircleCI] (SaaS, generic)::
  ...
  managed SaaS platform with first class support for a wide range of environments, it is fairly easy to get up and running
...
https://www.hashicorp.com/products/terraform/[Terraform Enterprise and Terraform Cloud] (self-hosted and SaaS, specialized)::
  ...
  `terraform`. Both do not support directly calling external binaries such as `terragrunt` or `docker` in a workflow.
  Terraform Cloud in particular does not support the use of external binaries even when called from within `terraform`
  using a `local-exec` provisioner or external data source. In terms of the threat model, Terraform Cloud and Enterprise
  both implement respectable security practices, including end to end encryption backed by HashiCorp Vault, and you have
  the option to run in your own data center with Terraform Enterprise to lock it down. Note that Terraform Enterprise
  relies on API based integrations with the VCS platforms instead of webhooks, so it is not necessary to have a publicly
  facing service when running in self-hosted mode.
...
- Use any generic CI/CD server you wish to integrate with your VCS repositories.
- Deploy a self-hosted deployment server within your AWS account that cannot be accessed by anyone but a few admins plus
  a limited trigger we'll define below.
...
Instead, we delegate this responsibility to an isolated, closed off system in the AWS account that only expose a limited
...
Terraform Enterprise::
  ...
  addition to licensing cost for using the service.
