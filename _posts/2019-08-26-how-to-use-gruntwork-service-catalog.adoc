...
You can now test manually by authenticate to your sandbox environment (see
...
terraform init -backend-config=testing-backend.hcl
...
First, create a `staging.tfvars` file:
...
.infrastructure-modules/networking/vpc-app/staging.tfvars
[source,hcl]
----
aws_region       = "us-east-2"
...
----
cd infrastructure-modules/networking/vpc-app
terraform apply -backend-config=staging-backend.hcl -var-file=staging.tfvars
----

To deploy to other environments, create analogous `.tfvars` and `.hcl` files (e.g., `production.tfvars` and
`production-backend.hcl`) and run `terraform apply` with those files.

[NOTE]
.Making the backend config more DRY
====
In each environment, you'll most likely want to use the same backend settings (e.g., S3 bucket, DynamoDB table)
for multiple modules. The only difference between the modules will be the `key` value, which needs to be unique for
each module. To reduce copy/paste, you could (1) create a `common/staging-backend.hcl` file with all the backend
settings for the staging environment except `key`, (2) create a `networking/vpc-app/backend.hcl` file that solely sets
the `key` parameter, and then (3) deploy as follows:

----
cd infrastructure-modules/networking/vpc-app
terraform apply \
  -backend-config=../../common/staging-backend.hcl \
...
* Manually setting a unique `key` for each module is repetitive and error prone.
* From a quick glance at the code, it's not clear what accounts, environments, or regions you deploy to. Figuring this
  out requires digging through every `.tfvars` file.
...
that helps with some of the drawbacks mentioned in the previous approach.
...
<account>::
  A the top level, you have accounts (e.g., an AWS account) or project (e.g., GCP project).
...
<environment>::
  Within each region, there will be one or more environments, such as dev, stage, prod, etc. There may also be a
  `_global` folder that defines resources that are available across all the environments in this region.

<resource>::
  Within each environment, you deploy one or more resources, such as servers, databases load balancers, and so on. The
  configuration for these resources is in a `terragrunt.hcl` file.
...
`infrastructure-live/production/us-east-2/prod/vpc-app/terragrunt.hcl` file. If you have multiple modules and you want
to deploy all of them, you can use `apply-all`. For example, to deploy everything in the production account, you would
do the following:
...
ip-lockdown::
  ...
  specific users can access them. The main motivation is to lock down the
  ...
  just any user who happens to get access to EC2 instance.
...
Once you've set this token, to install the `ip-lockdown` script and `ssh-grunt` binary, you run (make sure to replace
`<VERSION>` with the latest version from the
...
Now that you know _how_ to use Gruntwork Installer, let's talk about _where_ to use it.

==== Where to run the Gruntwork Installer

You will most likely want to use the Gruntwork Installer to install scripts and binaries on your EC2 instances. We
recommend following <<immutable_infrastructure>> practices and creating an immutable, versioned image that has the
scripts and binaries installed. There are many ways to do this, so in this guide, we'll focus on just the following
ones:

. <<gruntwork_install_packer>>
. <<gruntwork_install_docker>>

[[gruntwork_install_packer]]
===== Running the Gruntwork Installer in Packer templates

If you want to build machine images with scripts and binaries from the Gruntwork Service Catalog, such as an Amazon
Machine Image (AMI) for use in AWS, you can use https://www.packer.io[Packer]. Here's an example of how to use
`gruntwork-install` in a Packer template:
...
[[gruntwork_install_docker]]
===== Running the Gruntwork Installer in Docker images

If you want to build Docker images with scripts and binaries from the Gruntwork Service Catalog, the approach is
similar to what you saw earlier, but you have to take extra care with how you pass in your GitHub personal access
token. You should NOT hard-code the token into your `Dockerfile` (never store secrets in plain text) and you should NOT
naively pass it in via a Docker build argument, as build arguments get stored in the Docker image layers, and anyone
with access to that Docker image will be able to extract your token!

The safe way to pass secrets when building a Docker image is to use a
https://docs.docker.com/develop/develop-images/multistage-build/[multi-stage build], as shown in the following example:
...
----
docker build --build-arg GITHUB_OAUTH_TOKEN=(YOUR_TOKEN) -t example-image:v1 .
----
...
Note that the major drawback of copying these files is that you'll have to manually pull in new versions as we update
the code.
...
Tools::
  The main tools we expect customers to use are Terraform, Docker, and Packer, all of which work on all major operating
  systems.

Terraform modules::
  Just about all the Terraform modules in the Gruntwork Service Catalog work on all major operating systems. However,
  there are a handful of modules where we were forced to call out to scripts from our Terraform code. Most of these
