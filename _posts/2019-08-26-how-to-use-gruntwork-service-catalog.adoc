...
To build a machine image from a Packer template, you authenticate to a sandbox environment for testing
(see <<manual_tests_terraform>>) and run:
...
At the end of the build, Packer will output the ID of the image you've built (e.g., the AMI ID). You can then test the
...
Building machine images in the cloud (i.e., in AWS and GCP) can take a while, as you have to wait to fire up a virtual
server, run your provisioners, snapshot the server, shut it down, etc. This can add a few minutes of overhead to each
Packer build. One way to speed it up for local testing is to add a Docker builder to your Packer template that uses
the same base OS image. This will allow you to run the build completely locally _and_ run the Docker image itself
...
as explained in <<using_terraform_modules>>. For example, you can use the following modules:

* *AWS Auto Scaling Group (stateless):* use
  https://github.com/gruntwork-io/module-asg/tree/master/modules/asg-rolling-deploy[asg-rolling-deploy] in `module-asg`.
  Can do auto scaling, auto healing, and zero-downtime rolling deployment.
* *AWS Auto Scaling Group (stateful):* use
  https://github.com/gruntwork-io/module-asg/tree/master/modules/server-group[server-group] in `module-asg`.
  Can do auto healing, zero-downtime rolling deployment, and attach persistent EBS volumes and ENIs.
* *Single EC2 Instance:* use
  https://github.com/gruntwork-io/module-server/tree/master/modules/single-server[single-server] in `module-server`.
  Can attach persistent EBS volumes and ENIs. Note that single servers are single points of failure, with no automatic
  recover, so even for a single server, we generally recommend using one of the Auto Scaling Group modules instead.

IMPORTANT: You must be a https://gruntwork.io/[Gruntwork subscriber] to access `module-server` and `module-asg`.

All of these modules allow you to specify the machine image (i.e., AMI) to deploy via an input variable
(e.g., `ami_id`).
...
When using a `Dockerfile`, you should NOT hard-code your GitHub personal access token (or any other secret) into your
`Dockerfile` and you should NOT naively pass it in via a Docker build argument, as build arguments get stored in the
Docker image layers, and anyone with access to that Docker image will be able to extract your token! The safe way to
pass secrets when building a Docker image is to use a
...
. Login to your Docker Registry: `docker login`. Check your Registry's documentation for details.
...
Kubernetes::
  ...
  using https://github.com/gruntwork-io/terraform-aws-eks[terraform-aws-eks] and for GCP, we recommend using
  https://github.com/gruntwork-io/terraform-google-gke[terraform-google-gke].
+
...
Puppet, or Salt. After all, they are just scripts and binaries! Check the docs for your chose configuration management
