...
include blocks, which allowed you to have different tiers of common configuration. Before this change, you were only
limited to a single set of included configuration, which was typically used for project level configuration that
...
as VPCs and Databases, it was rare for the environment level deviation to be significant. This made it difficult to
manage the individual components across updates, as you needed to reflect the same changes in every environment, rather
...
the Refernece Architecture. This allows you to incrementally roll out your changes instead of updating all components at
once.
...
Given that, the order in which the components are updated do not matter. You can update the components in whatever order
you would like.
...
point and run the `diff` utility against it:
...
. For each `locals`, check if the local variable is used in the configuration. If you find no references for the given
...
. For each remaining `locals`, check if the local variable is used in the configuration. If you find no references for
...
[[further_steps]]
==== (Optional) Further DRY configuration
...
more planning and thinking than a step by step tutorial can offer, and thus we can not provide a step by step tutorial
