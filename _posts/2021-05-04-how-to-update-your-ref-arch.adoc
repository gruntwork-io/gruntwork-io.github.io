...
On https://blog.gruntwork.io/introducing-the-gruntwork-module-service-and-architecture-catalogs-eb3a21b99f70[August
26th, 2020, we announced the initial release] of the Service Catalog in a private, invite-only alpha program. Since then
we have expanded the catalog to be generally available, with all Gruntwork subscribers getting access to the services of
the catalog. A _service_ includes everything you need in production, including the Terraform code for provisioning,
Packer templates or Dockerfiles for configuration management, built-in monitoring (metrics, logging, alerting), built-in
security features (SSH access, secrets management, server hardening, encryption in transit and at rest), and so on. This
allows you to deploy infrastructure without writing any code.
...
maintenace from the end users to keep up to date with each change, including updating module references when new
versions were released.

With the Service Catalog, there is no longer a need to maintain a separate service module for every component you use.
...
some differences that require you to make some modifications to Terragrunt configurations in your `infrastructure-live`
...
* First, follow the migration guides published in
  ...
  `v0.0.1-20201125`.
* Prep your `infrastructure-live` repo with new Terragrunt configuration.
* From that point onward, follow the per-service guides here to update each service to the
  `terraform-aws-service-catalog` at `v0.35.4`.
