...
=== Enable AWS Config on the root account
...
----
terraform {
  source = "git@github.com/<YOUR_ORG>/infrastructure-modules.git//security/aws-config?ref=v0.3.1"
}
----
...
inputs = {
  ...
  iam_role_name = "<COMPANY_NAME>-aws-config-role"
...
=== Enable GuardDuty on the root account
