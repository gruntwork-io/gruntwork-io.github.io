...
=== Configure your Terraform remote state S3 bucket and DynamoDB lockfile
Now we need to configure the backend for your root account to store your terraform state in an s3 bucket and the dynamoDB table for the lockfile.
Create a terragrunt.hcl file in infrastructure-live/root.
...
infrastructure-live
 ...
    └── terragrunt.hcl
----
Replace <COMPANY_NAME> with your company name or another designation of your choosing.
...
----
 ...
 remote_state {
   ...
   config = {
     encrypt        = true
     bucket         = "<COMPANY_NAME>-root-terraform-state"
     key            = "${path_relative_to_include()}/terraform.tfstate"
     region         = "us-east-2"
     dynamodb_table = "<COMPANY_NAME>-root-terraform-locks"
   }
 }
 ...
 {
   aws_account_id             = "111111111111"  # < --- root account number
   terraform_state_s3_bucket  = "<COMPANY_NAME>-root-terraform-state"
   terraform_state_aws_region = "us-east-2"
 },
 )
----
We also need to add the file empty.yaml to infrastrucutre-live/root.
...
Place the contents below in the empty.yaml file.
...
.infrastructure-modules/security/organization/main.tf
[source,hcl]
----
provider "aws" {
  # The AWS region in which all resources will be created
...
.infrastructure-modules/security/organization/variables.tf
[source,hcl]
----
# ---------------------------------------------------------------------------------------------------------------------
# MODULE PARAMETERS
...
.infrastructure-modules/security/organization/outputs.tf
[source,hcl]
----
