...
Configure the https://www.terraform.io/docs/backends/[backend] you want to use to store your Terraform state.
...
As before, configure the backend you want to use, which with Terragrunt you can do by including the settings from the
root `terragrunt.hcl`:
...
analogous steps for EACH of your child accounts!
...
IAM user, making sure to set the MFA token on the CLI, and deploy the `cloudtrail` module by running
`terraform apply -var-file=<your-file>.tfvars` or, if using Terragrunt, by running `terragrunt apply`:
