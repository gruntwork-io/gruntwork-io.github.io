...
You'll want to enable CloudTrail, AWS Config, and Amazon GuardDuty in every single AWS account so that you are notified
of any anomalous activity (with GuardDuty) and that you have the audit logs (from CloudTrail and AWS Config) to
understand what happened.

For CloudTrail and AWS Config, we typically recommend creating an S3 bucket in the security account and sending all the
audit logs from the other accounts to this one S3 bucket. Also, make sure to encrypt all logs with KMS, and only give a
small number of trusted admins access to the KMS master key and the S3 bucket.
...
No security account (maybe)::
  ...
  for CloudTrail and AWS Config (e.g., perhaps them to the root account).
...
Security account::
  ...
  the account structure has multiple accounts, each person on your team will have only a single IAM user and a single
  set of credentials (the one exception is the root account, where one or a small number of admins will have a separate
  IAM user and separate credentials used to manage the account structure itself). You'll see later in this guide how
  to use IAM roles to allow a single IAM user in one AWS account to access other AWS accounts.
...
Resetting the root user password::
  ...
  follow the steps in <<lock_down_root_user>>. You should end up with a locked-down root user and a locked-down
  admin IAM user in each account: the root user you'll hardly ever use again; the IAM user in the security account
  you'll use as your



=== Clean up sandbox accounts

Sandbox accounts tend to build up more and more unused infrastructure over time, so to prevent your bill
from getting out of hand, you may want to run a tool like https://github.com/gruntwork-io/cloud-nuke[cloud-nuke] on
a daily basis to clean up everything in the account older than, say, 5 days (you wouldn't want to run a tool like
  `cloud-nuke` in an account like staging, so this is yet another reason to keep sandbox accounts separate).
