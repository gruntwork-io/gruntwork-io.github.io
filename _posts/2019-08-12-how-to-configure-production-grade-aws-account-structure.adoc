...
To use AWS, you sign up for an AWS account. To use AWS in production, you have to create an _AWS account structure_
that consists of multiple inter-connected AWS accounts.
...
Root permissions::
  ...
  only use the root user during initial setup to create IAM users (the topic of the next section) with more limited
  permissions, and then you'll likely never touch the root user account again.
...
Let's take a tour of this design.

=== The root account

At the top of the design, you have the root account of your AWS organization. No infrastructure runs in this account,
and only one or a small number of trusted admins should have IAM users in this account, using it solely to create and
manage child accounts and billing.
...
you need depends on your company's requirements, but for most companies, the root account contains solely a single
`full-access` IAM group that gives the handful of trusted users in that account admin permissions.
...
Security account::
  You will want a single _security account_ for managing authentication and authorization. No infrastructure runs in
  this account. Instead, this is where you define all of the IAM users and IAM groups for your team. None of the other
  child accounts will have IAM users; instead, those accounts will have IAM roles that can be assumed from the
  security account. That way, each person on your team will have a single IAM user and a single set of credentials
  in the security account (with the exception of the small number of admins who will also have a separate IAM user in
  the root account) and they will be able to access the other accounts by assuming IAM roles.
...
Application accounts (dev, stage, prod)::
  ...
  a separate account for backup and/or disaster recovery).
...
openvpn-allow-certificate-requests-for-external-accounts and openvpn-allow-certificate-revocations-for-external-accounts::
  These IAM roles allows users to request and revoke VPN certificates for an OpenVPN server running in the child
  account. This is part of the Gruntwork https://github.com/gruntwork-io/package-openvpn/[package-openvpn] code, which
  deploys a production-grade OpenVPN server, and gives developers with access to these IAM roles and the
  `openvpn-admin` tool a self-service VPN certificate experience.
+
IMPORTANT: You must be a https://gruntwork.io/[Gruntwork subscriber] to access
...
iam-user-self-mgmt::
  This IAM group grants permission to an IAM user to manage their own IAM user account, including resetting the
  password, generating access keys, and enabling MFA. It also grants permission to list other IAM users (so you can
  find your own user in the list), but not to view or modify any information about other users. Just about every IAM
  user should be added to this group or they likely won't be able to make much use of their own user account. For most
  IAM users, this will be the _only_ permission they have in the security account itself.
...
_account-<ACCOUNT>-<ROLE>::
  ...
  permissions to the `allow-full-access-from-other-accounts` IAM role in the dev account (so they will have full access
  to that account), users you add to the `\_account-prod-read-only` group will get `sts:AssumeRole` permissions to
  the `allow-read-only-access-from-other-accounts` IAM role in the prod account (so they will have read-only access to
  that account), and users you add to the `\_account-prod-openvpn-requests` group will get `sts:AssumeRole` permissions
  to the `openvpn-allow-certificate-requests-for-external-accounts` IAM role in the prod account (so they will be able
  to request OpenVPN certs for that account).
...
an easy way to enforce MFA globally, and if you try to enforce it in a naive manner, you'll run into issues: e.g., AWS
services, your own applications, and machine users won't be able to access your account and new IAM users won't be able
to access the account to configure an MFA token in the first place.
...
IAM users and groups::
  ...
  The only exception is the `iam-user-self-mgmt` IAM group, where you need one policy that will allow a user to enable
  MFA in the first place; this is the only policy that will not require MFA.

=== Password policy

In any account that has IAM users (which should just be the root and security accounts), configure a password policy
that ensures all IAM users have strong passwords. The exact policy you use depends on your companies requirements (e.g.,
...
allow-auto-deploy-access-from-other-accounts::
  ...
  some specific service. You may have many such roles, depending on what types of services you deploy automatically.
  For example, this role may have a trust policy that allows it to be assumed by a Jenkins server in the
  shared-services account, and gives that server permissions to deploy EC2 Instances and Auto Scaling Groups.

allow-ssh-grunt-access-from-other-accounts::
  This is an IAM role that grants permission to look up IAM group membership and the public SSH keys on IAM user
...
----
+
Notice that the `Principal` is set to `"ec2.amazonaws.com"`. Each AWS service has its own `Principal`: e.g., if you
...
give a small number of trusted admins access to the KMS master key and the S3 bucket.
...
No security account (maybe)::
  You might be able to get rid of the security account entirely. The key considerations are (a) if you need to have any
  machine users (if so, perhaps define them in the shared-services account?) and (b) where to store your audit logs
  for CloudTrail (e.g., perhaps them to the root account).
...
No MFA::
  One other big difference with IAM roles for federated auth is that these roles should NOT require an MFA token.
...
// TODO: extract the Terraform/Terragrunt/infra-live/infra-modules content into a separate guide and cross-link it
// instead of copy/pasting it into each guide
Git repos::
...
to (a) create IAM users, IAM groups, and IAM roles that you will use instead, as we'll discuss now and (b) lock down
the root user account as much as possible, as we'll discuss a little later.

Let's first create the IAM groups and IAM users by using the `iam-groups`, `iam-users`, `iam-user-password-policy`, and
`cross-account-iam-roles` modules from https://github.com/gruntwork-io/module-security[module-security].
...
.infrastructure-modules/networking/iam-groups/main.tf
[source,hcl]
----
module "iam_groups" {
...
Create all the corresponding input variables for `iam-gruops` in `variables.tf`:

.infrastructure-modules/networking/iam-groups/variables.tf
[source,hcl]
----
variable "aws_region" {
...
And the corresponding variables for the `iam-users` module in `variables.tf`:

.infrastructure-modules/networking/iam-users/variables.tf
[source,hcl]
----
variable "users" {
...
And the corresponding input variables in `variables.tf`:
...
// TODO: I'm skipping the testing steps for this module here, but we should cross-link here to our Terraform/Terragrunt
// guide when it's ready

Commit and release your changes:
...
If you using Terraform directly, you will be running commands directly in `infrastructure-modules`. If you are using
Terragrunt, you will need to create a `terragrunt.hcl` file in `infrastructure-live`. It should go under the file path
`root/\_global/iam`:
...
Set the variables for the `iam` module in this environment. You can set these values in a `.tfvars` file or, if
using Terragrunt, in the `inputs = { ... }` block of `terragrunt.hcl`:
...
Configure the backend https://www.terraform.io/docs/backends/[backend] you want to use to store your Terraform state.
The https://www.terraform.io/docs/backends/types/s3.html[S3 backend] is usually the best choice for AWS accounts. If
you're using Terragrunt, the backend settings should be defined in a root `terragrunt.hcl` file that you `include` in
each child `terragrunt.hcl`:
...
Finally, deploy the `iam` module by running `terraform apply -var-file=<your-file>.tfvars` or, if using Terragrunt, by
...
Next, let's enable CloudTrail in the root account so we have an audit log of everything we do. You can do this using
the `cloudtrail` module from https://github.com/gruntwork-io/module-security[module-security].

IMPORTANT: You must be a https://gruntwork.io/[Gruntwork subscriber] to access `module-security`.

First, create a _wrapper module_ called `cloud` in your `infrastructure-modules` repo:
...
Next, use the `cloud-trail` module from the Gruntwork Service Catalog, making sure to replace the `<VERSION>` placeholder
with the latest version from the https://github.com/gruntwork-io/module-security/releases[releases page]:
...
Set the variables for the `cloudtrail` module in this environment. You can set these values in a `.tfvars` file or, if
using Terragrunt, in the `inputs = { ... }` block of `terragrunt.hcl`:
...
Since you already deleted the root user's access keys, this time, you should authenticate as your IAM user, making
sure to set the MFA token on the CLI! See
...
Finally, deploy the `cloudtrail` module by running `terraform apply -var-file=<your-file>.tfvars` or, if using
Terragrunt, by running `terragrunt apply`:
...
accounts detailed in <<child_accounts>>, but feel free to adjust this as necessary based on the accounts your company
needs.
...
And the corresponding output variables in `outputs.tf`:
...
Set the variables for the `organization` module in this environment. You can set these values in a `.tfvars` file or, if
using Terragrunt, in the `inputs = { ... }` block of `terragrunt.hcl`:
...
Authenticate on the CLI as your IAM user and deploy the `organization` module by running
`terraform apply -var-file=<your-file>.tfvars` or, if using Terragrunt, by running `terragrunt apply`:
...
If you using Terraform directly, you will be running commands directly in `infrastructure-modules`. If you are using
Terragrunt, you will need to create a `terragrunt.hcl` file in `infrastructure-live`. It should go under the file path
`<ACCOUNT>/\_global/iam`, where `<ACCOUNT>` is one of these other child accounts, such as dev, stage, prod, and
shared-services. In the rest of this example, we'll look solely at the stage account, but make sure you follow the
analogous steps for ALL of your child accounts!
...
Now that you have an IAM user in the security account and IAM roles in the other accounts, it's time to practice
authenticating!

. Use your IAM user's user name and password (decrypted using keybase) to log into the web console of the security
  account.
. Follow the steps in <<lock_down_iam_users>>.
. After creating an MFA token, log out, and then log back into the web console of the security account with an MFA
  token. If you don't do this, the next steps won't work!
...
Now that you can access all the child accounts using your IAM user and IAM roles, you should follow the steps in
<<lock_down_root_user>> for the root user of each of those child accounts, and (almost) never use those root users
again.
...
If you using Terraform directly, you will be running commands directly in `infrastructure-modules`. If you are using
Terragrunt, you will need to create a `terragrunt.hcl` file in `infrastructure-live`. It should go under the file path
`<ACCOUNT>/\_global/cloudtrail`, where `<ACCOUNT>` is one of these other child accounts, such as dev, stage, prod, and
shared-services. In the rest of this example, we'll look solely at the stage account, but make sure you follow the
analogous steps for ALL of your child accounts!
...
Since you already deleted the root user's access keys, this time, you should authenticate as your IAM user, making
sure to set the MFA token on the CLI, _and_ assume a role in the stage account! See
https://blog.gruntwork.io/a-comprehensive-guide-to-authenticating-to-aws-on-the-command-line-63656a686799[A Comprehensive Guide to Authenticating to AWS on the Command Line]
for instructions on how to do that.
