...
. Create an S3 bucket in the logs account and grant the CloudTrail service in all accounts write access to this
  bucket.
...
. (Optional) Configure CloudTrail to also send logs to CloudWatch Logs within each account. This gives you an
  additional way to quickly view recentl CloudTrail data for an account in the account itself, without having to login
  to the logs account.
...
inputs = {
  ...
  # On the first deploy, we must disable AWS Config and CloudTrail, as we want to send AWS Config and CloudTrail data
  # to the logs account, but before we can do that, we have to create and configure the logs account!
...
. **Disable AWS Config and CloudTrail for the first deployment**. We want to send all AWS Config and CloudTrail data to
  the logs account, but that child account doesn't exist yet, so we need to initially disable AWS Config and CloudTrail,
  create the logs account and apply a security baseline to it, and then we'll come back to the root account and
  re-enable AWS Config and CloudTrail.

. **Create IAM groups**. We create a `full-access` IAM group (for admins) and a `billing` IAM group (for the finance
  team).

. **Create IAM users**. For this example, we create `alice`, `bob`, and `carol`. We add `alice` and `bob` to the
  `full-access` IAM group and `carol` to the `billing` IAM group.

. **Generate a password for each user**. We encrypt this password with that user's PGP key from Keybase (we'll come
  back to how to handle the passwords shortly). You should follow this pattern to create an IAM user for yourself, as
  well as the small number of other trusted admins at your company who should have access to the root account.
...
. **Aggregate CloudTrail Logs**: We create an S3 bucket and a KMS CMK and grant all accounts permissions to encrypt
  their CloudTrail llogs with the CMK and store those logs in the S3 bucket.
...
Now that your root and logs accounts are fully configured, you need to apply the security baseline to the security
account.
...
Authenticate once more to the root account and run `terragrunt apply`:
