...
This section will cover how to use Gruntwork in conjunction with two popular HashiCorp products: Terraform Cloud (TFC)
and Terraform Enterprise (TFE). Although the open source edition of Terraform is quite powerful and flexible as a
standalone project, many organizations turn to TFC/TFE for the CLI/UI integration, approval-based workflow capabilities,
Sentinel policy framework, and more.

It's easy to use the production-grade, battle-tested Terraform modules from Gruntwork's infrastructure-as-code library
to deploy infrastructure with TFC and TFE. With a little more effort, you can also use Terragrunt! Throughout this
section, to avoid repeating ourselves, we'll only mention TFC. The same patterns work just as well with TFE.

[NOTE]
.Obtain an API token
====
Before working with TFC/TFE from your local system, you'll need to obtain an API token. You can generate an API token in your user settings of the TFC UI. Follow https://www.terraform.io/docs/commands/cli-config.html#credentials-1[the official instructions] to use the token.
====

==== Using a Gruntwork module in a TFC workspace

One of the first tasks to undertake in TFC is to create a workspace, which is a backend location where Terraform state
is kept. Workspaces are usually linked to VCS repositories that contain the Terraform code that defines your infrastructure.
In a typical TFC workspace, after linking your VCS repository, you add Terraform variables to customize your
environment, then use the built-in plan and apply features to manage the infrastructure. Under the hood, TFC clones the
repository, sets up the variables according to your definitions, and runs Terraform against the configuration. Each run
is tracked and available for review, and the state is stored centrally within TFC. 

When you're using Gruntwork modules, you use the source attribute on a module block to read a module from a Gruntwork
code repository. As a simple example, if you wanted to create an Amazon SNS topic using the SNS module from https://github.com/gruntwork-io/package-messaging[`package-messaging`], you might create something like the following:
...
module "sns" {
  source = "git::git@github.com:gruntwork-io/package-messaging.git//modules/sns?ref=v0.3.2"
  name         = "my-topic"
  display_name = "MyTopic"
}
----

The `git::git@github.com:gruntwork-io` portion of the `source` attribute indicates that this module is accessed over SSH.
Therefore, to access the Gruntwork module, the system running Terraform (the TFC executor in this case) will need permissions to clone the repository
via SSH.

Fortunately, creating a workspace that uses a Gruntwork module is easily accomplished in TFC. Take the following steps:
...
executor. Create a new Github user, and email the username to mailto:support@gruntwork.io[support@gruntwork.io]. We'll make sure the user has access to our repositories.

1. Next, add an SSH key to the new GitHub machine user. GitHub has https://help.github.com/en/enterprise/2.19/user/github/authenticating-to-github/connecting-to-github-with-ssh[easy-to-follow instructions].

1. Now, add the private portion of the SSH key to TFC. You'll find the option under SSH Keys in the TFC organization settings. We called ours _Gruntwork access_.

1. Set up a new workspace and link it to your code repository that uses Gruntwork's module as a source.

1. In the Workspace settings, in the SSH key section, you can now choose the Gruntwork access key.

.Configuring a TFC Workspace to access Gruntwork repositories
image::/assets/img/guides/infrastructure-as-code-library/tfc_ssh_key.png[]


When TFC runs your module, it will use the configured SSH key to clone the Gruntwork repository during the run, and will continue with creating the SNS topic you defined. You can reuse this SSH key across any other workspaces that depend on Gruntwork modules.

That's it! You can use this pattern for any of Gruntwork's Terraform modules. This even works for modules like github.com/gruntwork-io/terraform-aws-eks[`terraform-aws-eks`] which need to download external dependencies (like https://github.com/gruntwork-io/kubergrunt[`kubergrunt`]). Those modules use the special https://github.com/gruntwork-io/package-terraform-utilities/tree/master/modules/executable-dependency[`executable-dependency`] module to install the external dependency within the executor at run time.

In the future, Gruntwork may offer modules through  a private Terraform registry to further simplify the process.

[[terragrunt_tfc_tfe]]
==== Using Terragrunt with TFC and TFE
Configuring Terragrunt to work with TFC/TFE is a bit more involved. As mentioned previously, TFC and TFE execute
`terraform` commands under the hood, and the executors that run these commands aren't able to execute the Terragrunt
binary. Running Terragrunt directly through the TFC UI isn't an option.

However, Terragrunt can rely on TFC's https://www.terraform.io/docs/cloud/run/cli.html#implicit-workspace-creation[implicit workspace creation] feature to automatically create workspaces when Terragrunt runs. This feature allows you to use Terragrunt to invoke TFC runs from your local CLI. Once you're up and running, the state will be maintained in TFC like any other workspace, and the standard plan and apply workflow will function normally. There are several steps involved to make this work:

1. Generate a `backend` block to use TFC/TFE for remote state storage and execution.
1. Generate a `terraform.auto.tfvars` for submitting variables.
1. Configure authentication to providers.

In the sections below we will cover each step in turn.

===== Setting up

Consider the following Terragrunt directory structure:
...
For simplicity, the example shows a single environment, `dev`, with two modules: `sqs` and `sns`. The pattern could be expanded to more environments and regions by adding to the hierarchy as needed.

===== Configuring the backend

For Terragrunt to use TFC as a remote executor, you'll need a `backend` block. The backend is what determines where Terraform reads state and how execution is handled. In this case, you'll configure TFC as a remote in the backend block. The Terragrunt https://terragrunt.gruntwork.io/docs/reference/config-blocks-and-attributes/#generate:[`generate` block] can generate the block dynamically so you don't need to include one in each module.

Terraform https://www.terraform.io/docs/state/workspaces.html[workspaces] store the state associated with a given backend. In the Terragrunt configuration hierarchy defined above, each module (`sns` and `sqs`) is a separate workspace and must have its own name in the `backend` block. There are several ways to do this with Terragrunt. The snippet below shows one approach: use the account name (defined in `account.hcl`) as a prefix, and use the directory name of the module as the suffix.

The following snippet is placed in `terragrunt.hcl` at the root of the hierarchy.

[source,hcl]
----
locals {
  tfc_hostname     = "app.terraform.io"
  tfc_organization = "gruntwork-demo"
...
terraform {
  backend "remote" {
    ...
    workspaces {
      name = "${local.account_vars.locals.account}-${local.workspace}"
    }
  }
}
EOF
...
This snippet shows off a few features of Terragrunt:

* The `generate` block creates a file called `backend.tf` that will be created alongside the other `*.tf` files for the module that Terragrunt is operating on. This effectively adds a dynamic `backend` configuration to the module.
* Terragrunt has a set of https://terragrunt.gruntwork.io/docs/reference/built-in-functions/[build-in functions], like `find_in_parent_folders()`, `read_terragrunt_config()`, and `get_terragrunt_dir()`. These functions are used to configure the backend.
* Terragrunt can also use all of the https://www.terraform.io/docs/configuration/functions.html[native Terraform functions]. In the configuration above, we're using https://www.terraform.io/docs/configuration/functions/split.html[`split()`] and https://www.terraform.io/docs/configuration/functions/reverse.html[`reverse()`] to obtain the leaf directory name (such as `sqs` and `sns`) that is used as the workspace suffix.
...
The following `terragrunt.hcl`, located in the `sqs` directory, shows how to generate a  `terragrunt.auto.tfvars` file.
...
Any of the inputs needed by the module must be included in the generated tfvars file. In the configuration above, only the `name` variable is specified. Most modules will need more configuration.


===== Configure provider authentication

When TFC runs `terraform` commands that interact with remote APIs, it must authenticate to the provider. For example, when using the AWS provider, you need to https://www.terraform.io/docs/providers/aws/index.html#authentication[set the AWS API credentials]. There are a few ways to do this, each with trade offs.

The first way is to set two environment variables in the TFC workspace: `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY`. This is perhaps the simplest approach, because all you need to do to set it up is to generate the access keys for an IAM user, assign that user appropriate permissions, and https://learn.hashicorp.com/terraform/tfc/tfc_workspaces#editing-variables[set the environment variables]. The downside is that the credentials are static. You'll need to create each workspace in advance, rather than relying on the implicit workspace creation, and for each workspace, you'll have to set the environment variables manually. If you rotate the keys, you'll have to update them in each workspace.

A second way is to generate a `provider` block in the root `terragrunt.hcl`, and include the static API credentials in the block. For example:
...
locals {
  ...
  access _key  = "<value for access key>"
...
Using this approach, the workspace does not need to be created in advance, and the environment variables do not need to be set. It's more scalable, but now the API keys are kept in the configuration in plain text. Because this configuration is stored in version control, this approach is not recommended.

A third way is to generate the provider block, but to retrieve dynamic credentials using the https://docs.aws.amazon.com/STS/latest/APIReference/API_AssumeRole.html:[AWS AssumeRole feature] and the Terragrunt https://terragrunt.gruntwork.io/docs/reference/built-in-functions/#run_cmd[`run_cmd` function]. Using this approach, the `run_cmd` invokes an external script to assume an IAM role, fetch credentials, and return them to Terragrunt, where they are subsequently inserted in to the provider configuration.
...
locals {
  ...
  role_arn     = "<your IAM role>"
  creds        = split(",", chomp(run_cmd("bin/get_aws_creds.py",
                       "--profile-name",
                       "${local.profile_name}",
                       "--role-arn",
                       "${local.role_arn}")))
  access_key     = local.creds[0]
  secret_key     = local.creds[1]
  session_token  = local.creds[2]
}
...
In this example, a simple Python script called `get_aws_creds.py` is invoked to assume the role defined in `local.role_arn`. The credentials are returned a comma-separated value of the form `access_key,secret_key,session_token`. The `split()` function separates the values in to a list whose elements are then used in the provider configuration.

In this case, the credentials used by the provider are temporary and not stored in the configuration directly. This avoids the security pitfalls associated with the second approach, and yet still leverages the implicit workspace creation, avoiding the need to set up workspaces and environment variables in advance. If you'd like more information about this configuration, please email mailto:support@gruntwork.io[support@gruntwork.io].


===== Summarizing Terragrunt and TFC/TFE compatibility

Using the combination of features above, you can effectively use Terragrunt with TFC or TFE as a remote backend. Terragrunt can generate backend blocks, tfvars files, and provider configurations, resulting in dynamic workspace creation. Commands like `terragrunt apply-all` will operate in the same way as they do without TFC/TFE, calling each module dependency in order, and allowing for passing outputs between modules.

IMPORTANT: The https://gruntwork.io/reference-architecture/[Gruntwork Reference Architecture] is not built for use with TFC or TFE. Although the reference architecture is deployed using Terragrunt, the state is stored in S3, and the configurations discussed above are not included. Migrating from the standard Gruntwork reference architecture to TFC is not supported.
