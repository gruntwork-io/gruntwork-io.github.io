...
Use TFC with Terragrunt::
  ...
  deployments. In this approach, TFC is used a https://www.terraform.io/docs/backends/types/remote.html:[remote backend]
...
After doing so, you'll be presented with the option to create a new organization. We suggesing using lowercase
...
https://www.terraform.io/docs/state/workspaces.html[Workspaces] store the state associated with the
infrastructure that is managed by Terraform. The state associated with your infrastructure is stored in
TFC. You can use the https://www.terraform.io/docs/providers/terraform/d/remote_state.html[`remote_state` data source]
to use the outputs of one workspace as the variable inputs for another workspace. In this manner, you can link multiple
workspaces together to build an end-to-end infrastructure.
...
`infrastructure-modules/modules`. In this structure, each workspace will use the same `infrastructure-modules` repository, but pointed at different subdirectories within the repository.

We'll demonstrate how to set up a workspace for a simple SQS module. To get started, create a new workspace in the TFC UI using the steps below.

===== Connect to a version control provider
Connect the workspace to the version control system of your choice. For example, GitHub, GitLab, or Bitbucket. This allows TFC to access your `infrastructure-modules` repository. Once your VCS is connected, find your `infrastructure-modules repository from the list of repositories presented.
...
consistent yet flexible naming convention.

We also need to inform TFC where to find the correct module. Under the _Advanced options_, locate the _Terraform Working Directory_ field, and enter the path to the module. TFC will automatically enter the same value for _Automatic Run Triggering_ to ensure that Terraform runs for this workspace only occur when changes are detected to the module path. For our SQS example, we enter the value `/modules/networking/sqs`.
...
clone the `infrastructure-modules` repository, download the AWS provider, download the Gruntwork SQS module, run a
`terraform plan`, wait for confirmation, and once confirmed, run a `terraform apply`.
...
github.com/gruntwork-io/terraform-aws-eks[`terraform-aws-eks`] which need to download external dependencies (like
...
Therefore, it currently is not possible to use the TFC UI to trigger Terragrunt. However, you can use TFC to run
...
TFC rather than your own computer.
...
* Terragrunt has a set of https://terragrunt.gruntwork.io/docs/reference/built-in-functions/[build-in functions], like `find_in_parent_folders()`, `read_terragrunt_config()`, and `get_terragrunt_dir()`. These functions are used to obtain values need to configure the backend.
...
In all cases, you'll need to ensure that your workspaces stay in sync with your Terragrunt configuration. Each time you add a new module in Terragrunt, you'll need a corresponding workpace.

Furthermore, if you rotate your AWS API keys, you'll need to update them within each workspace. For that reason, the final option above is recommended.
